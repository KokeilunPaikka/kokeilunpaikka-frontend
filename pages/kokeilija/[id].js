...
import { getSingleUser, getStatusOptions } from 'store/actions/users'
...
class SingleExperimenter extends Component<Props> {
  static async getInitialProps({ store, query }) {
    const { id } = query
    await store.dispatch(getSingleUser(id))
      ...
      <MainLayout>
        ...
        <Container>
          ...
          {experiments.length > 0 ? (
            <Row>
              <Col>
                <Title>{t('user-experiments')}</Title>
              </Col>
            </Row>
          ) : null}
